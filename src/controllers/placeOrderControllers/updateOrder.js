...
const updateOrder = async (req, res) => {
    let { couponCode={}, isCouponClaimed=false, paymentDetails, tableId, orderId } = req.body
    const { _id, role } = req.userDetails;
    // orderId = ObjectId(String(orderId));
    // tableId = ObjectId(String(tableId));
    console.log({ couponCode, isCouponClaimed, paymentDetails, tableId , orderId})
    // {
    //     "orderDetail": {
    //         "customer": {
    //             "name": "",
    ...
    try{
      if(resultRole)
      {
        ...
        if(orderFind){
          ...
          if(tableUpdate){
            response(res, 200, true, "Order Completed Successfully", {tableStatus:tableUpdate});
          }
        }else{
